...
Fretboard::Fretboard()
{
	Update(scale_length, fret_count, fret_width, fret_height, scallop_depth, show_scallop);
}

void Fretboard::Update(double scale_length, int fret_count, double fret_width, double fret_height, double scallop_depth, bool show_scallop)
{
	...
	pos = svg_data.find("\nCLIP_DATA");
	if (pos != std::string::npos)
		svg_data.replace(pos, std::string("\nCLIP_DATA").length(), CalcClipData());
...
std::string Fretboard::CalcClipData()
{
	...
	if (pos != std::string::npos)
		clip_data.replace(pos, std::string("CLIP_ID").length(), scallop_clip_id);

	pos = clip_data.find("\nCLIP_HEAD");
	if (pos != std::string::npos)
		clip_data.replace(pos, std::string("\nCLIP_HEAD").length(), CalcScallopClipHead());
...
std::string Fretboard::CalcScallopClipHead()
{
	...
	if (pos != std::string::npos)
		scallop_clip_head.replace(pos, std::string("SCALLOP_OFFSET").length(), "500");
...
std::string Fretboard::CalcScallopClipTail()
{
	...
	if (pos != std::string::npos)
		scallop_clip_tail.replace(pos, std::string("SCALLOP_OFFSET").length(), "500");
